...
MATCHES = {
    '<strong>For details about functions sort(), sorted(), min() and max() see duck type <a href="#sortable">sortable</a>.</strong>': '<strong>For details about functions sort(), sorted(), min() and max() see duck type sortable (p. 16).</strong>',
    '<strong>Listed <a href="#lambda">lambda expressions</a> can be replaced by the <a href="#operator">operator\'s</a> itemgetter() function.</strong>': '<strong>Listed lambda expressions can be replaced by the operator\'s itemgetter() function (p. 31).</strong>',
